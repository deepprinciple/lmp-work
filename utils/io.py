...
def _normalize_coeff_line(section: str, line: str) -> str:
    """Strip inline style names from coeff lines for non-hybrid inputs.

    Some generators emit entries like ``1 harmonic 300.0 1.5`` in ``Bond Coeffs``.
    When the input script already sets ``bond_style harmonic``, LAMMPS expects the
    data line to be ``1 300.0 1.5`` instead.
    """
    if section not in {"bond_coeffs", "angle_coeffs", "dihedral_coeffs", "improper_coeffs"}:
        return line
...
def parse_lammps_data(data_file: Path) -> Dict[str, List[str]]:
    ...
    for line in lines:
        ...
        if stripped and not stripped.startswith('#'):
            data[section].append(_normalize_coeff_line(section, line))
