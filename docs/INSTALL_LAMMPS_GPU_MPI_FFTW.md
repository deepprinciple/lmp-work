# LAMMPS 环境搭建手册（GPU + OpenMPI + FFTW3，禁用 KOKKOS）
...
```bash
conda create -n gkff python=3.11 -y
conda activate gkff
...
- `FFT library = FFTW3 ...`
- 包列表包含 `KSPACE/MOLECULE/EXTRA-*`

并且你**不需要**依赖 `KOKKOS`（本手册默认禁用）。
