...
OPENFF_LAMMPS_SETTINGS: Dict[str, Any] = {
    ...
    "special_bonds": {
        "lj": [0.0, 0.0, 0.5],
        "coul": [0.0, 0.0, 0.8333333333],
    },
    "kspace_style": "pppm 1e-5",
}


def get_openff_lammps_settings() -> Dict[str, Any]:
    """Return a copy of the default OpenFF-compatible LAMMPS settings."""
    return copy.deepcopy(OPENFF_LAMMPS_SETTINGS)
...
class OpenFF(ForceField):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            charge_file: 外部电荷文件路径（提供时覆盖内置电荷分配）
            use_cache: 是否启用参数缓存
        """
        ...
        self.charge_file = Path(charge_file).expanduser().resolve() if charge_file else None
    ...
    def generate_parameters(
        ...
        meta = {
            ...
            "charge_file": str(self.charge_file) if self.charge_file else None,
    ...
    def _make_cache_key(self, canonical_smiles: str) -> str:
        payload = {
            ...
            "charge_file_sha256": self._charge_file_digest(),
        }
    ...
    def _assign_charges(self, off_mol) -> str:
        if self.charge_file is not None:
            self._assign_external_charges(off_mol)
            return self.charge_method
    ...
    def _assign_external_charges(self, off_mol) -> None:
        try:
            import numpy as np
            from openff.units import unit
        except ImportError as exc:
            raise RuntimeError("外部电荷覆盖需要 openff.units 与 numpy") from exc

        charges = self._load_external_charges()
        if len(charges) != off_mol.n_atoms:
            raise RuntimeError(
                f"外部电荷数与原子数不一致: charges={len(charges)}, atoms={off_mol.n_atoms}"
            )

        charge_array = np.asarray(charges, dtype=float)
        total_charge = float(charge_array.sum())
        if abs(total_charge) > 1.0e-6:
            charge_array = charge_array - total_charge / len(charge_array)

        off_mol.partial_charges = charge_array * unit.elementary_charge

    def _load_external_charges(self) -> list[float]:
        if self.charge_file is None:
            raise RuntimeError("未提供外部电荷文件")
        if not self.charge_file.exists():
            raise FileNotFoundError(f"外部电荷文件不存在: {self.charge_file}")

        suffix = self.charge_file.suffix.lower()
        if suffix == ".json":
            payload = json.loads(self.charge_file.read_text())
            if isinstance(payload, list):
                charges = payload
            elif isinstance(payload, dict):
                if "partial_charges" in payload:
                    charges = payload["partial_charges"]
                elif "charges" in payload:
                    charges = payload["charges"]
                else:
                    raise RuntimeError(
                        f"JSON 电荷文件缺少 'partial_charges' 或 'charges' 字段: {self.charge_file}"
                    )
            else:
                raise RuntimeError(f"无法解析 JSON 电荷文件: {self.charge_file}")
        else:
            charges = []
            for raw in self.charge_file.read_text().splitlines():
                line = raw.strip()
        ...
        try:
            return [float(x) for x in charges]
        except Exception as exc:
            raise RuntimeError(f"外部电荷文件数值解析失败: {self.charge_file}") from exc

    def _charge_file_digest(self) -> str | None:
        if self.charge_file is None:
            return None
        if not self.charge_file.exists():
            return f"missing:{self.charge_file}"
        return hashlib.sha256(self.charge_file.read_bytes()).hexdigest()
    ...
    def _print_forcefield_info(self, mol: Chem.Mol, charge_method: str):
        ...
        if self.charge_file is not None:
            print(f"    外部电荷文件: {self.charge_file}")
