...
class PackmolBuilder:
    ...
    def build_box(
        ...
        Args:
            ...
            density: Initial packing density used to size the box (g/cm^3).
        ...
        Returns:
            Tuple of (system_xyz_path, (Lx, Ly, Lz)).
        """
        if n_molecules <= 0:
        ...
        # Compute box size from the initial packing density.
        box_dims = self._calculate_box_lengths(
        ...
        print(f"    初始装箱密度: {density:.3f} g/cm³")
        print(f"    盒子尺寸: {lx:.3f} x {ly:.3f} x {lz:.3f} Å")
        print(f"    Packmol tolerance: {tolerance:.3f} Å")
        print(f"    初始随机种子: {seed}")
        ...
        inner_lengths = (lx - 2.0 * margin, ly - 2.0 * margin, lz - 2.0 * margin)

        if not full_box and min(inner_lengths) <= 0:
            raise RuntimeError(
                f"盒子尺寸过小({lx:.3f}, {ly:.3f}, {lz:.3f} Å)，"
    ...
    def _calculate_box_lengths(
        *,
        ...
        """
        Compute orthorhombic box lengths.
        ...
        if box_lengths is not None:
            ...
            if min(lx, ly, lz) <= 0:
                raise ValueError("box_lengths 每个分量都必须 > 0")
            return lx, ly, lz

        mass_total = mol_weight * n_molecules / AVOGADRO  # g
        volume_cm3 = mass_total / density  # cm³
        volume_a3 = volume_cm3 * 1e24  # Å^3

        if box_aspect_ratio is None:
            box_aspect_ratio = (1.0, 1.0, 1.0)
        if len(box_aspect_ratio) != 3:
            raise ValueError("box_aspect_ratio 必须包含 3 个分量")
        rx, ry, rz = [float(v) for v in box_aspect_ratio]
        if min(rx, ry, rz) <= 0:
            raise ValueError("box_aspect_ratio 每个分量都必须 > 0")
    ...
    def _write_packmol_input(
        ...
        margin: float,
        inner_lengths: tuple[float, float, float],
        tolerance: float,
        seed: int,
        *,
        full_box: bool = False,
        ...
        lx, ly, lz = box_dims
        inner_lx, inner_ly, inner_lz = inner_lengths
        if full_box:
            region_line = (
            ...
            region_line = (
                ...
                f"{margin + inner_lx:.6f} {margin + inner_ly:.6f} {margin + inner_lz:.6f}"
            )
